...
发送 `菜单` 后，可以直接在飞书卡片里查看当前计划摘要，并使用 `查询额度`、`设置明天计划`、`立即预热` 三个入口。当前计划会分开展示今日和明天的预热节点；已执行、未执行、失败、已取消的节点会有明确状态。

![菜单与当前计划](docs/images/menu-and-current-plan.png)

### 自动编排明日计划

只需要选择一个开始时间，系统会按当前可用额度优先选择一个 AI 工具，并生成两次预热节点。默认 auto 策略会尽量吃满单个工具两段 5 小时窗口，形成约 7.5 小时、接近 200% 的重点使用区间。采用计划前，你仍然可以调整两次预热时间。已有明天计划时，再点 `设置明天计划` 可以查看当前计划，并为另一个可规划模型补充预热。
