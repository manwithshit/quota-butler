...
- 调度安全降级：生成计划和采用计划时都会复核 provider；不可用 Agent 自动剔除，
  旧卡片在采用前二次拦截。
- 逐步规划交互：已完成任务类型、工作强度、原生时间选择器、偏好摘要和调整入口。
- 计划卡产品化：已改为人话摘要与覆盖率/空档/接力次数，不再展示 `CAS` 缩写。
- 计划版本：新计划使用 `plan_version: 2`，旧计划拒绝采用。
- 预热安全：普通开窗、one-up 和计划定时任务均只允许 Codex。
- 移动端布局：选择和摘要按钮统一使用最多两列的网格。
- 多人配置：bridge 注入经过验证的操作者 `open_id`，handler 按用户保存日常场景。
- 首次使用：先展示手动输入场景表单，再进入四类任务按钮。
- 流程版本：引导卡升级为 V3，旧 V2 引导卡不可继续。
...
- 总览：任务类型、工作强度、工作时间、实际使用的 Agent
- 计算依据：计划覆盖率、预计空档、预计接力次数
- 时间线：
  - Agent 预热节点
  - Agent 恢复节点
  - 用户创作窗口
...
- `schedule_flow.py` 负责纯偏好数据和流程版本校验。
- `planner.py` 生成真实窗口事件与非执行性的建议接力点。
- `notify.py` 将节点解释为任务相关的人话目的。
- `handler.py` 负责逐步 callback 和 Agent 自动选择。
